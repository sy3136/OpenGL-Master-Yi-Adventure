...
std::vector<vertex> create_grass_vertices() {
	...
	v.push_back({ vec3(1, 1, 2), vec3(1, 1, 2).normalize(), vec2(1.0f, 1.0f) });
	v.push_back({ vec3(-1, -1, 0), vec3(-1, -1, 0).normalize(), vec2(0.0f, 0.0f) });
	v.push_back({ vec3(1, 1, 0), vec3(1, 1, 0).normalize(), vec2(1.0f, 0.0f) });
...
void update_grass_vertex_buffer(const std::vector<vertex> vertices, GLuint& vertex_array)
{
