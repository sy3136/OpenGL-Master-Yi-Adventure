...
bool init_title(const char* image_path, GLuint& SRC, GLuint& vertex_array_image) {
	// Image
	// create a frame buffer object for render-to-texture
	// create corners and vertices
	vertex corners[4];
	corners[0].pos = vec3(-1.78f, -1.0f, 0.0f);	corners[0].tex = vec2(0.0f, 0.0f);
	corners[1].pos = vec3(+1.78f, -1.0f, 0.0f);	corners[1].tex = vec2(1.0f, 0.0f);
	corners[2].pos = vec3(+1.78f, +1.0f, 0.0f);	corners[2].tex = vec2(1.0f, 1.0f);
	corners[3].pos = vec3(-1.78f, +1.0f, 0.0f);	corners[3].tex = vec2(0.0f, 1.0f);
